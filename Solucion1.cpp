...
struct Solucion1 {
    ...
    int Buscar( uchar* palabra)  { //la funcion recive un array 
        ...
        while (inicio <= fin) { //empieza una busqueda binaria
            ...
            if (comp == 0) { //si encontramos la palabra 
                cout << "la palabra se encuentra en el indice " << mitad << " del array!\n";
                //avisa que se encontro la palabra y en que indice
                return mitad;  
            } else if (comp == -1) {
                inicio = mitad + 1; //la palabra es mas grande que la mitad
            } else {
                fin = mitad - 1; // la palabra es mas pequeña que la mitad
            }
        }
    ...
    void insertar(uchar* nuevaPalabra) { //se le da una palabra
        if (Contenido == Capacidad) { //verifica si hay espacio
            expandir(); //expande el array si no hay espacio
        }
    ...
    int eliminar( uchar* palabra) { //recive una palabra a eliminar
        int indice = Buscar(palabra); // verifique si existe
        
        if (indice == -1) { //la palabra no existe
            cout << "la palabra no esta en el array!\n" //avisa que no existe
            return 0; //hace un return para acabar el programa antes
        }

        for (int i = indice; i < Contenido - 1; i++) { //ocupa el indice de la funcion de busqueda
            Array[i] = Array[i + 1]; //mueve todas las palabras drente a la palabra una celda hacia atras
        } //cabe decir que mientras puede que queden palabras al final del array, como el numero de palabras
        // en el array evita que esas palabras sean ocupadas erroneamente no causa problema alguno
